                          ...
                          include 'DB_Connection.php';
                            ...
                            if (isset($_POST["dbtn"])) {
                                ...
                                $sql = "SELECT log_table.log_id, 
                                        ...
                                        WHERE Attend_Date = '$dte'
                                        GROUP BY log_table.Student_ID";

                                $result = $conn->query($sql);

                                if ($result->num_rows > 0){
                                  echo "<table class='center' id='allTable' style='border:4px solid black;Font-size:12px;Font-Weight:bold;background:white;'>
...
function ExportToExcel(type, fn, dl) {
	var elt = document.getElementById('allTable');
	var wb = XLSX.utils.table_to_book(elt, { sheet: "sheet1" });
	return dl ?
		XLSX.write(wb, { bookType: type, bookSST: true, type: 'base64' }) :
		XLSX.writeFile(wb, fn || ('All_Students_Report.' + (type || 'xlsx')));
}
</script>
